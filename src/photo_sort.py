...
class PhotoSort:
    ...
    def copy(self):
        ...
        for src_path in files:
            ...
            trg_path = self.trg_path_of(src_path, src_info)
    ...
    def read_photos_info(self) -> dict[str, dict]:
        ...
        for json_path in json_files:
            with open(json_path, 'r') as f:
                ...
                file_name = os.path.basename(json_path).rstrip(".json").rstrip(".supplemental-metadata")
                json_dict[file_name] = json_value
        return json_dict

    def trg_path_of(self, src_path, info: dict = None):
        ...
        if info:
            ...
            if name.startswith("CIMG") or name.startswith("IMAG"):
                trg_name = str(dt) + pathlib.Path(src_path).suffix.lower()
            year = str(dt.year)
        else:
        ...
        if is_autogen_folder:
            trg_path = str(Path(self.trg_dir, year, trg_name))
        else:
            album_name = Path(src_path).parent.name
            trg_path = str(Path(self.trg_dir, year, album_name, trg_name))
